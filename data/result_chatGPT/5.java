/**
 * This method is used to generate random index settings.
 *
 * @param indexName The name of the index.
 * @return The generated settings.
 */
private Settings generateIndexSettings(String indexName) {
...
/**
 * This method is used to generate index metadata.
 *
 * @param indexName The name of the index.
 * @param indexSettings The settings of the index.
 * @param indexState The state of the index.
 * @return The generated index metadata.
 */
private IndexMetadata generateIndexMetadata(String indexName, Settings indexSettings, IndexMetadata.State indexState) {
...
/**
 * This method is used to generate index routing table.
 *
 * @param index The index.
 * @param numberOfShards The number of shards.
 * @param numberOfReplicas The number of replicas.
 * @return The generated index routing table.
 */
private IndexRoutingTable.Builder generateIndexRoutingTable(Index index, int numberOfShards, int numberOfReplicas) {
    IndexRoutingTable.Builder indexRoutingTable = IndexRoutingTable.builder(index);
    switch (randomFrom(ClusterHealthStatus.values())) {
        case GREEN:
            addShards(index, numberOfShards, indexRoutingTable, "nodeA", true, ShardRoutingState.STARTED);
            if (numberOfReplicas > 0) {
                addShards(index, numberOfShards, indexRoutingTable, "nodeB", false, ShardRoutingState.STARTED);
            }
            break;
        case YELLOW:
            addShards(index, numberOfShards, indexRoutingTable, "nodeA", true, ShardRoutingState.STARTED);
            if (numberOfReplicas > 0) {
                addShards(index, numberOfShards, indexRoutingTable, null, false, ShardRoutingState.UNASSIGNED);
            }
            break;
        case RED:
            break;
...
/**
 * This method is used to add shards to the index routing table.
 *
 * @param index The index.
 * @param numberOfShards The number of shards.
 * @param indexRoutingTable The index routing table.
 * @param node The node.
 * @param primary Whether the shard is primary.
 * @param state The state of the shard.
 */
private void addShards(Index index, int numberOfShards, IndexRoutingTable.Builder indexRoutingTable, String node, boolean primary, ShardRoutingState state) {
    IntStream.range(0, numberOfShards)
        .mapToObj(n -> new ShardId(index, n))
        .map(shardId -> TestShardRouting.newShardRouting(shardId, node, primary, state))
        .forEach(indexRoutingTable::addShard);
}
/**
 * This method is used to generate index stats.
 *
 * @return The generated index stats.
 */
private IndexStats generateIndexStats() {
...
public void testBuildTable() {
    ...
    for (int i = 0; i < numIndices; i++) {
        ...
        Settings indexSettings = generateIndexSettings(indexName);
        ...
        if (frequently()) {
            IndexMetadata indexMetadata = generateIndexMetadata(indexName, indexSettings, indexState);
            metadata.put(indexMetadata, false);
            if (frequently()) {
                Index index = indexMetadata.getIndex();
                IndexRoutingTable.Builder indexRoutingTable = generateIndexRoutingTable(index, indexMetadata.getNumberOfShards(), indexMetadata.getNumberOfReplicas());
                routingTable.add(indexRoutingTable);
                if (frequently()) {
                    IndexStats indexStats = generateIndexStats();
    ...
    verifyTable(table, clusterState, indicesStats);
}
/**
 * This method is used to verify the table.
 *
 * @param table The table to verify.
 * @param clusterState The cluster state.
 * @param indicesStats The indices stats.
 */
private void verifyTable(Table table, ClusterState clusterState, Map<String, IndexStats> indicesStats) {
