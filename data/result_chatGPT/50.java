...
private void disassembleRegion(PrintWriter out) {
    ...
    DumbMemBufferImpl memBuffer = getMemoryBuffer();
    ParallelInstructionLanguageHelper helper = getParallelInstructionHelper();
    int cnt = 0;
...
private Disassembler getDisassembler() {
    return Disassembler.getDisassembler(currentProgram, false, false, false, monitor, null);
}
/**
 ...
 */
private DumbMemBufferImpl getMemoryBuffer() {
    return new DumbMemBufferImpl(currentProgram.getMemory(), region.getMinAddress());
}
/**
 * Gets the parallel instruction helper.
 *
 * @return the parallel instruction helper.
 */
private ParallelInstructionLanguageHelper getParallelInstructionHelper() {
    return currentProgram.getLanguage().getParallelInstructionHelper();
}
/**
