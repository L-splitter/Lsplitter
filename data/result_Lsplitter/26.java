...
protected final void assertInvariants(Map<K, V> map) {
  ...
  assertCollections(valueCollection, entrySet, map);
...
/** 
 * Asserts the invariants of the value collection and the entry set.
 * @param valueCollection the value collection to check
 * @param entrySet the entry set to check
 * @param map the map to check
 */
private void assertCollections(Collection<V> valueCollection, Set<Entry<K,V>> entrySet, Map<K,V> map){
  // Asserts for value collection
